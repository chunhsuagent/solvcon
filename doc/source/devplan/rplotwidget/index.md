...
1. **Step 1. Series and model (C++).** `RPlotSeries` (a `SimpleArray<double>` pair plus style) and `RPlotModel` (the series list, the color cycle, the NaN-safe data limits, and the view limits with an autoscale margin and a nonsingular guard). The model derives a `ViewTransform2dFp64` from its limits rather than adding a new affine. Consumers reach the samples only through the series' own accessor -- the current point count and the current x/y spans, fetched where they are used -- never by keeping a copy of the stored `SimpleArray`, and the series carries a revision counter that the cached data limits and the decimated point set key off. `SimpleArray` is the only input this plan accepts, deliberately, but that is a limit to lift later rather than an assumption to bake in: the accessor and the revision are what let a variable-size source (`SimpleCollector`) become a series without touching the model, the ticker, the decimator, or the widget (see "Room for a variable-size data source"). Pure C++ math with no Qt widget; bound through pybind11 and covered by pytest the way the rest of the pilot is tested.
...
class RPlotSeries {
    ...
    // label, color, width
    uint64_t m_revision = 0;        // bumped on every data change
public:
    void set_data(SimpleArray<double> x, SimpleArray<double> y);
    // The only way a consumer reads samples: the current extent, fetched
    // where it is used, so a growing source stays correct. Nothing
    // downstream keeps the array itself.
    size_t size() const;
    std::span<double const> x() const;
    std::span<double const> y() const;
    uint64_t revision() const;      // caches key off this and recompute
...
### Room for a variable-size data source

`SimpleArray` is a fixed-extent view, so a series built on one cannot follow data that is still being produced. The pilot already has the growable counterpart: `SimpleCollector<T>` (`cpp/solvcon/buffer/SimpleCollector.hpp`, bound to Python as `SimpleCollectorFloat64` and its siblings) is a `push_back` / `reserve` / `pop_back` buffer over `BufferExpander`. Accepting one as a series source is what makes a live plot possible -- a residual history, a probe time series, or a convergence curve that grows while the solver runs -- and it is a follow-up rather than part of this train. What this plan owes it is a design that does not foreclose it, which is why Step 1 puts sample access behind the series' accessor and gives the series a revision counter.

The trap that design avoids is worth recording, because it fails silently. `SimpleCollector::as_array()` goes through `BufferExpander::as_concrete()`, which materializes the current contents into a `ConcreteBuffer` and returns a `SimpleArray` over it -- a snapshot, not a live view. A later `push_back` that outgrows the capacity calls `BufferExpander::reserve()`, which allocates fresh storage, copies into it, and drops the concrete buffer (`cpp/solvcon/buffer/BufferExpander.cpp:11`), so the collector writes elsewhere from then on. A snapshot the plot still holds keeps its own reference, so nothing dangles, but it is frozen: new samples never appear, and its extent was fixed when it was taken. A series that stored that array by value would show a stale prefix forever with no error to notice. The other cost to respect is that `as_array()` is not free when the collector is not already concrete -- `copy_concrete` copies the whole buffer -- so the eventual implementation wants a zero-copy path to the collector's current span, not an `as_array()` call per frame.

Two questions belong to that follow-up rather than to this plan: whether a growing series re-decimates from scratch each frame or extends the previous reduction, and whether autoscale keeps chasing the incoming data or pins the view so a streaming plot does not jump under the user's cursor.
...
- `SimpleCollector`, or any variable-size container, as a series source, together with the live streaming plot it enables; Step 1 keeps the seam for it and "Room for a variable-size data source" records the snapshot and per-frame-copy pitfalls to avoid.
